...
export const Homeworks = () => {
    ...
    useEffect(() => {
        if (data) {
            ...
            if (userHomeworks) {
                const filteredHomeworks = filterArray(
                    userHomeworks,
                    status.completed,
                    status.isChecking,
                    status.notCompleted
    ...
    }, [data, status, user.course]);

    function filterArray(array, filter1, filter2, filter3) {
        const completedHomeworks = filter1
            ? array.filter(homework =>
                  homework.completed.find(completedData => {
                      if (completedData.user === user.email) {
                          return completedData.status === 'Completed';
                      }
                  })
              )
            : [];
        ...
        const isCheckingHomeworks = filter2
            ? array.filter(homework =>
                  homework.completed.find(completedData => {
                      if (completedData.user === user.email) {
                          return completedData.status === 'Is checking';
                      }
                  })
              )
            : [];
        ...
        const notCompletedHomeworks = filter3
            ? array.filter(homework =>
                  homework.completed.find(completedData => {
                      if (completedData.user === user.email) {
                          return completedData.status === 'Not completed';
                      }
                  })
              )
            : [];
